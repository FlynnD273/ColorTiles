...
func set_tile(pos: Vector2i, type: TILE_TYPE) -> void:
  set_cell(pos, 0, Vector2i(type, 0))
...
func generate_tiles() -> void:
  ...
  const rand_tiles: Array[TILE_TYPE] = [
	TILE_TYPE.YELLOW, TILE_TYPE.ORANGE, TILE_TYPE.BLUE, TILE_TYPE.RED, TILE_TYPE.PURPLE
  ]
  ...
  const yellow_replace: Array[TILE_TYPE] = [
	TILE_TYPE.PINK, TILE_TYPE.ORANGE, TILE_TYPE.BLUE, TILE_TYPE.RED, TILE_TYPE.PURPLE
  ]
...
func _get_raw_tile_type(pos: Vector2i) -> TILE_TYPE:
  var tile: int = get_cell_atlas_coords(pos).x
  if tile < 0:
