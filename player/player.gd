...
@onready var gameboard: TileMapLayer = $"../Gameboard"
...
var directions = {"ui_right": 0, "ui_left": 180, "ui_up": -90, "ui_down": 90}
...
func _ready() -> void:
  TILE_WIDTH = gameboard.tile_set.tile_size.x * gameboard.scale.x
...
func automove(dir: Vector2i) -> void:
  ...
  (
	move_tween
	. tween_property(self, "position", new_pos, speed)
	. set_trans(Tween.TRANS_QUAD)
	. set_ease(Tween.EASE_IN_OUT)
  )

  if (
	type == TILE_TYPE.YELLOW or (type == TILE_TYPE.BLUE and flavour == FLAVOURS.ORANGE)
  ):
	next_move = -dir
